...
similar_pkgs = matched_names.map(i => {
    ...
    return htl.html`<li><a href="/orgmetrics-ropensci/repo.html?repo=${i}" onclick="localStorage.setItem('orgmetricsRepo', '${i}')">${i}</a></li>`
});
```
...
<li><a href='/orgmetrics-ropensci/contributor.html'>${auts_gh}</a></li>
</div>
` : auts_gh.map(m => htl.html`
<div onclick=${() => localStorage.setItem('orgmetricsMaintainer', m)}>
<li><a href='/orgmetrics-ropensci/contributor.html'>${m}</a></li>
</div>
`));
...
<a href='/orgmetrics-ropensci/contributor.html'>${item}</a>${i < ctbs_gh.length - 1 ? ", " : ""}</span>`);
...
<li>
<a href='/orgmetrics-ropensci/repo.html?repo=${d}' onclick="localStorage.setItem('orgmetricsRepo', '${d}')">${d}</a>
</li>`);
